...
import type { Chyt } from '@/lib/database.types'
...
interface ChytFormProps {
  agents: Pick<Agent, 'id' | 'name'>[]
  repos: Pick<AgentRepo, 'id' | 'agent_id' | 'repo_url'>[]
  contracts?: Pick<Project, 'id' | 'name' | 'is_default'>[] | null
  workOrder?: Chyt | null
  isEdit: boolean
}
...
export function ChytForm({
  ...
  useEffect(() => {
    if (workOrder) {
      setContractId(workOrder.project_id ?? '')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (isEdit && workOrder?.id) {
        const res = await fetch(`/api/chyts/${workOrder.id}`, {
          method: 'PATCH',
        ...
        router.push(`/chyts/${workOrder.id}`)
        router.refresh()
      } else {
        const res = await fetch('/api/chyts', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        })
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
      <div className="flex items-center gap-2">
        <Button type="button" variant="ghost" size="icon" asChild>
          <Link href={workOrder ? `/chyts/${workOrder.id}` : '/chyts'}>
