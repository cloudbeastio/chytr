...
import type { ProjectStatRow } from '@/lib/database.types'

interface ProjectStatsProps {
  stats: ProjectStatRow | null
...
export function ProjectStats({ stats, budgetLimit }: ProjectStatsProps) {
  if (!stats) {
    return (
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
      <Card>
        <CardHeader className="pb-1">
          <CardTitle className="text-xs font-medium text-muted-foreground">Chyts</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-lg font-semibold tabular-nums">{stats.total_chyts}</p>
