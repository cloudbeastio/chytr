...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ ok: true, followup_message }),
...
function normalizePayload(eventType: string, raw: Record<string, unknown>): Record<string, unknown> {
  switch (eventType) {
    case 'tool_call':
    case 'tool_result':
      return {
        ...
        result_length: raw.result_length ?? (typeof raw.result === 'string' ? raw.result.length : null),
      }
    case 'tool_failure':
      return {
        tool_name: raw.tool_name ?? raw.toolName ?? '',
    ...
    case 'shell_execution':
      return {
        ...
        stdout_length: typeof raw.stdout === 'string' ? raw.stdout.length : (raw.stdout_length ?? 0),
        stderr_length: typeof raw.stderr === 'string' ? raw.stderr.length : (raw.stderr_length ?? 0),
...
async function checkDefinitionOfDone(
  ...
  const lines = wo.lines as Array<{ id: string; title: string; status?: string; definition_of_done?: string }>
