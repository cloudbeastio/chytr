...
import { ChytFilters } from '@/components/chyts/filters'
import { cn } from '@/lib/utils'
import { ClipboardList, Plus } from 'lucide-react'
import type { Chyt, ChytStatus, ChytSource } from '@/lib/database.types'
...
async function ChytTable({
  ...
  const workOrders = (data ?? []) as ChytRow[]
  ...
  const { data: toolCallLogs } = await supabase
    ...
    .in('chyt_id', workOrderIds)

  const toolCallCounts = (toolCallLogs as { chyt_id: string | null }[] ?? []).reduce<
    Record<string, number>
  >((acc, log) => {
    if (log.chyt_id) {
      acc[log.chyt_id] = (acc[log.chyt_id] ?? 0) + 1
    }
    return acc
  }, {})
            ...
            return (
              ...
              >
                <TableCell className="max-w-[240px]">
                  <Link href={`/chyts/${wo.id}`} className="block">
...
export default async function ChytsPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="space-y-5 max-w-full">
      ...
      >
        <ChytTable status={status} source={source} />
      </Suspense>
    </div>
  )
}
