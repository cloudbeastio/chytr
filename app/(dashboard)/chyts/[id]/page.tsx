...
import { ChytActions } from '@/components/chyts/chyt-actions'
import type { Chyt, AgentLog, ChytStatus, ChytSource } from '@/lib/database.types'

const STATUS_CLASSES: Record<ChytStatus, string> = {
...
const STATUS_LABELS: Record<ChytStatus, string> = {
  draft: 'Draft',
  pending: 'Pending',
  running: 'Running',
  completed: 'Completed',
...
const SOURCE_CLASSES: Record<ChytSource, string> = {
...
export default async function ChytDetailPage({ params }: PageProps) {
  const { id } = await params
  const supabase = await createSupabaseServerClient()

  const { data: woData, error } = await supabase
    .from('chyts')
    .select('*, agents!agent_id(name), agent_repos!repo_id(repo_url), projects!project_id(id, name)')
    .eq('id', id)
    .single()

  if (error || !woData) notFound()

  const wo = woData as ChytDetail
  ...
  const lines = Array.isArray(wo.lines) ? (wo.lines as ChytLine[]) : null
      ...
      <div className="px-6 pt-5 pb-4 shrink-0 space-y-3">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <ChytActions workOrderId={id} status={wo.status} />
            <Badge
              variant="outline"
